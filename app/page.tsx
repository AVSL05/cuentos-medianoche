...
export default function Home() {
  ...
  const togglePlay = () => {
    ...
    if (audioRef.current.paused) { audioRef.current.play(); setPlaying(p => p); }
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setNewTitle(file.name.replace(/\.mp3$/i, ''));
  };
          ...
          <div style={{
            ...
            >
              ...
              <p style={{ color: selectedFile ? 'var(--gold)' : 'var(--lavender)', fontSize: 15 }}>
                {selectedFile ? selectedFile.name : 'Toca para seleccionar un archivo MP3'}
              </p>
              {selectedFile && (
            ...
            >
              {uploading ? 'Guardando...' : 'Guardar cuento'}
            </button>
          </div>
        )}
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                  ...
                  <div style={{
                    ...
                    {playing === story.id && !audioRef.current?.paused ? '⏸' : '▶'}
                  </div>
